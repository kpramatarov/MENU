 ...
 *  ----------------------------------------------------------------------------
 *  ВАРИАНТ ЗА ИЗМЕРВАНЕ: версия 2.1 без промени в логиката, с добавено само
 *  измерване на времето за цикъл с micros() (редове с "[ИЗМЕРВАНЕ]"). След всеки
 *  пакет телеметрия по UART се извежда ред:
 *    V2LOOP,<средно µs>,<макс. µs>,<брой итерации>,<прозорец ms>
 *  Служи за сравнение "преди/след" с версия 3 по един и същ метод.
 *  ----------------------------------------------------------------------------
 *  Версия 2.1 - добавени:
...
// [ИЗМЕРВАНЕ] време за изпълнение на loop() с micros() за прозореца между два пакета
uint32_t v2Count = 0, v2MaxUs = 0, v2WindowStartMs = 0;
uint64_t v2SumUs = 0;
...
// ИЗМЕРВАНЕ НА A0 С ОСРЕДНЯВАНЕ НА 10 ПРОБИ ЗА ИЗГЛАЖДАНЕ НА ШУМА
  ...
  // [ИЗМЕРВАНЕ] извеждане и нулиране на прозореца
  Serial.printf_P(PSTR("V2LOOP,%lu,%lu,%lu,%lu\n"),
                  (unsigned long)(v2Count ? v2SumUs / v2Count : 0), (unsigned long)v2MaxUs,
                  (unsigned long)v2Count, (unsigned long)(millis() - v2WindowStartMs));
  v2Count = 0; v2SumUs = 0; v2MaxUs = 0; v2WindowStartMs = millis();
}
...
void loop() {
  uint32_t v2StartUs = micros();                     // [ИЗМЕРВАНЕ]
  ...
  uint32_t v2Dt = micros() - v2StartUs;              // [ИЗМЕРВАНЕ]
  v2Count++; v2SumUs += v2Dt;
  if (v2Dt > v2MaxUs) v2MaxUs = v2Dt;
}
