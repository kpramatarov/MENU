...
за едновременно обслужване на уеб сървър, ESP-NOW обмен, четене на два датчика (DHT11 и
HR202) и четири релейни канала, без ресурсът да е прекомерен; това обосновава изискването за **напълно
...
3. **Разработване на принципна електрическа схема на подчинения възел**, включващо:
   - анализ на съвместимостта на релейните модули с оптронен вход с 3,3 V логика и избор
     на активното ниво;
   - свързване на двата датчика – DHT11 за температура и влажност и HR202 за влага – и на
     бутоните, със
...
7. **Реализация на неблокиращ главен цикъл** – неблокиращо обслужване на бутоните и на
   ...
   блокировка при отчетена влага (датчик HR202) и измерване на времето за цикъл.
...
- **Не се реализира измерване на общия ток на присъединяването.** Съгласно изходните данни
  по заданието измервателната част включва датчик за температура – в реализацията това е DHT11
  (температура и влажност), допълнен с датчика за влага HR202; въвеждането на
...
натискания на бутоните с 1…6 отскока през 0,1…1,6 ms [26]; намокряне на датчика за влага
...
Затихването в сграда се описва с логаритмичния модел [39, 40]
$PL(d) = PL(1\ \text{m}) + N \lg d + L_{\text{прегр}}$, където $PL(1\ \text{m}) = 40{,}2$ dB при
2437 MHz, а $N \approx 28$ за жилищни сгради в обхвата 2,4 GHz [40]. От бюджета се
...
4. Филтърът на бутоните приема всички нормални натискания и отхвърля всички почуквания под
   60 ms. Защитната блокировка се задейства при всяко намокряне на HR202, отказва всички опити за
