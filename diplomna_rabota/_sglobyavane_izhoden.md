...
библиотеката `EEPROM` [24] емулира такава чрез отделен сектор от SPI Flash паметта, който се
...
радиостека** [12, 31]. Затова в тях е недопустимо да се извършват продължителни операции, работа
...
Статичното съдържание на потребителския интерфейс се съхранява в **LittleFS** [32] – файлова
...
Интерфейсът е изграден съгласно архитектурния стил **REST** [33].
...
Използването на **Fetch API** [34] с `async/await` осигурява **асинхронно обновяване без
...
Библиотеката `DallasTemperature` [35] по подразбиране изчаква завършването на преобразуването
...
За закрити помещения се прилага логаритмичният модел [36, 37]:
...
**Стандартната неопределеност по тип Б** се извежда от каталожната граница ±0,5 °C [38] при
приемане на равномерно разпределение:

$$u_B = \frac{0{,}5}{\sqrt{3}} = 0{,}289\ ^\circ\text{C}$$

Съгласно [39] **комбинираната** и **разширената** неопределеност при коефициент на покритие $k = 2$
