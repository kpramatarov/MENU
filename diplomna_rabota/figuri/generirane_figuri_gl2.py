...
def fig_2_1():
    ...
    s += blk(px, 126, pw, 44, 'Модул HR202 + LM393', 'наличие на течност · 3,3 V')
...
def fig_2_5():
    ...
    # HR202 и R1 - делител
    ax, ay = 170, 245
    s += res_v(ax, yv, ay, 'HR202', 'RH ↑ → R ↓', 'l', w=16, h=44) + dot(ax, yv)
    ...
    s += T(520, 378, 'течност: UA > Uпраг → DO = 0 (светва HL1)', 10.5, it=True)
    ...
    s += caption(W, 504, 'Фиг. 2.5. Типична схема на модула HR202 с компаратор LM393')
...
def fig_2_7():
    ...
    # датчици и бутони
    s += R(430, 720, 170, 110, 1.6) + T(515, 740, 'BK1 – DHT11', 10.5, True)
    for k, (lbl, nn) in enumerate((('VCC', '+3,3 V'), ('DATA', 'D4'), ('GND', 'GND'))):
        py = 764 + k * 24
        s += T(438, py + 4, lbl, 9.5, an='start') + net(430, py, nn, 'l', 14)
    s += R(660, 720, 190, 110, 1.6) + T(755, 740, 'A5 – HR202 + LM393', 10.5, True)
