...
def fig_3_1():
    ...
    s_items = [('handleSafetyCutoff()', 'защита при влага'), ('handleButtons()', 'бутони, филтър 60 ms'),
...
def fig_3_6():
    ...
    s += T(380, 100, 'DO = 0 (влага) /', SM, it=True)
    ...
    s += label_lines(36, 414, ['В състоянията на блокировка applyRelay(…, включено) се отказва и броячът на отказите',
                               'се увеличава; изключването е разрешено винаги. DO – изход на датчика за влага HR202.'],
    ...
    s += caption(H, 'Фиг. 3.6. Краен автомат на защитната блокировка при влага')
...
def fig_3_8():
    ...
    loop = [('proc', ['t₀ = micros(); период спрямо', 'предходната итерация']),
            ('sub', ['handleSafetyCutoff()', '– HR202, защита (Фиг. 3.6)']),
